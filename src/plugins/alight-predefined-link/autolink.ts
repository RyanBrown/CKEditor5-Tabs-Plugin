 ...
 * The autolink plugin.
 */
export default class AlightPredefinedLinkAutoLink extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return [Delete, AlightPredefinedLinkEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightPredefinedLinkAutoLink' as const;
  }
  ...
  private _expandLinkRange(model: Model, position: Position): Range | null {
    if (position.textNode && position.textNode.hasAttribute('alightPredefinedLinkHref')) {
      return findAttributeRange(position, 'alightPredefinedLinkHref', position.textNode.getAttribute('alightPredefinedLinkHref'), model);
    } else {
      return null;
    }
  }
  ...
  private _enablePasteLinking(): void {
    ...
    const AlightPredefinedLinkCommand = editor.commands.get('link')!;

    clipboardPipeline.on('inputTransformation', (evt, data: ClipboardInputTransformationData) => {
      if (!this.isEnabled || !AlightPredefinedLinkCommand.isEnabled || selection.isCollapsed || data.method !== 'paste') {
      ...
      if (matches && matches[2] === newLink) {
        model.change(writer => {
          ...
          AlightPredefinedLinkCommand.execute(newLink);
        });
  ...
  private _persistAutoLink(url: string, range: Range): void {
    ...
    model.enqueueChange(writer => {
      writer.setAttribute('alightPredefinedLinkHref', url, range);
...
function isLinkAllowedOnRange(range: Range, model: Model): boolean {
  return model.schema.checkAttributeInSelection(model.createSelection(range), 'alightPredefinedLinkHref');
}

function linkIsAlreadySet(range: Range): boolean {
  const item = range.start.nodeAfter;
  return !!item && item.hasAttribute('alightPredefinedLinkHref');
}
