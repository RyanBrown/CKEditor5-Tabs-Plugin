...
/**
 * The unlink command. It is used by the {@link module:link/link~AlightPredefinedLink link feature}.
 * Enhanced to remove organization names from links.
 */
export default class AlightExternalUnlinkCommand extends Command {
  /**
   * @inheritDoc
   */
   ...
   *
   * When the selection is collapsed, it removes the `alightPredefinedLinkHref` attribute from each node with the same `alightPredefinedLinkHref` attribute value.
   * When the selection is non-collapsed, it removes the `alightPredefinedLinkHref` attribute from each node in selected ranges.
  ...
  public override execute(): void {
    ...
    const model = this.editor.model;
    const selection = model.document.selection;
    const linkCommand = editor.commands.get('alight-predefined-link') as AlightPredefinedLinkCommand | undefined;
    ...
    model.change(writer => {
      ...
      const rangesToUnlink = selection.isCollapsed ?
        ...
        model.schema.getValidRanges(selection.getRanges(), 'alightPredefinedLinkHref');

      // Remove `alightPredefinedLinkHref` attribute from specified ranges.
      for (const range of rangesToUnlink) {
        ...
        writer.removeAttribute('alightPredefinedLinkHref', range);
