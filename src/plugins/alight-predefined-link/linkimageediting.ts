...
/**
 * @module link/AlightPredefinedLinkImageediting
 */
 ...
 *
 * It accepts the `alightPredefinedLinkHref="url"` attribute in the model for the {@link module:image/image~Image `<imageBlock>`} element
 * which allows linking images.
 */
export default class AlightPredefinedLinkImageEditing extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return ['ImageEditing', 'ImageUtils', AlightPredefinedLinkEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightPredefinedLinkImageEditing' as const;
  }
  ...
  public afterInit(): void {
    ...
    if (editor.plugins.has('ImageBlockEditing')) {
      schema.extend('imageBlock', { allowAttributes: ['alightPredefinedLinkHref'] });
    }

    editor.conversion.for('upcast').add(upcastLink(editor));
    editor.conversion.for('downcast').add(downcastImageLink(editor));

    // Definitions for decorators are provided by the `link` command and the `AlightPredefinedLinkEditing` plugin.
...
function upcastLink(editor: Editor): (dispatcher: UpcastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<UpcastElementEvent>('element:a', (evt, data, conversionApi) => {
      ...
      // There's an image inside an <a> element - we consume it so it won't be picked up by the AlightPredefinedLink plugin.
      ...
      const alightPredefinedLinkHref = viewLink.getAttribute('href');

      // Missing the 'href' attribute.
      if (!alightPredefinedLinkHref) {
        return;
      }
      ...
      if (modelElement && modelElement.is('element', 'imageBlock')) {
        // Set the alightPredefinedLinkHref attribute from link element on model image element.
        conversionApi.writer.setAttribute('alightPredefinedLinkHref', alightPredefinedLinkHref, modelElement);
      }
    }, { priority: 'high' });
...
function downcastImageLink(editor: Editor): (dispatcher: DowncastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>('attribute:alightPredefinedLinkHref:imageBlock', (evt, data, conversionApi) => {
...
function downcastImageLinkManualDecorator(decorator: ManualDecorator): (dispatcher: DowncastDispatcher) => void {
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${decorator.id}:imageBlock`, (evt, data, conversionApi) => {
      ...
      // It may happen when the base `alightPredefinedLinkHref` and decorator attributes are removed
