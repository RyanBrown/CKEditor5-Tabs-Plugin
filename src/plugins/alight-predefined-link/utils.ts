...
/**
 * A keystroke used by the {@link module:link/linkui~AlightPredefinedLinkUI link UI feature}.
 */
...
/**
 * Returns `true` if the specified `element` can be linked (the element allows the `alightPredefinedLinkHref` attribute).
 */
export function isLinkableElement(element: Element | null, schema: Schema): element is Element {
  ...
  return schema.checkAttribute(element.name, 'alightPredefinedLinkHref');
}
