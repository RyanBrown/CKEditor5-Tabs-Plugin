...
import AlightPredefinedLinkCommand from './linkcommand';
import AlightPredefinedLinkUnlinkCommand from './unlinkcommand';
 ...
 *
 * It introduces the `alightPredefinedLinkHref="url"` attribute in the model which renders to the view as a `<a href="url">` element
 * as well as `'link'` and `'unlink'` commands.
 */
export default class AlightPredefinedLinkEditing extends Plugin {
  /**
   * @inheritDoc
   */
  public static get pluginName() {
    return 'AlightPredefinedLinkEditing' as const;
  }
  ...
  public init(): void {
    ...
    editor.model.schema.extend('$text', { allowAttributes: 'alightPredefinedLinkHref' });

    editor.conversion.for('dataDowncast')
      .attributeToElement({ model: 'alightPredefinedLinkHref', view: createLinkElement });

    editor.conversion.for('editingDowncast')
      .attributeToElement({
        model: 'alightPredefinedLinkHref', view: (href, conversionApi) => {
    ...
    editor.conversion.for('upcast')
      .elementToAttribute({
        ...
        model: {
          key: 'alightPredefinedLinkHref',
          value: (viewElement: ViewElement) => viewElement.getAttribute('href')
        }
      });

    // Create linking commands.
    editor.commands.add('alight-predefined-link', new AlightPredefinedLinkCommand(editor));
    editor.commands.add('alight-predefined-unlink', new AlightPredefinedLinkUnlinkCommand(editor));
    ...
    // Enable two-step caret movement for `alightPredefinedLinkHref` attribute.
    const twoStepCaretMovementPlugin = editor.plugins.get(TwoStepCaretMovement);
    twoStepCaretMovementPlugin.registerAttribute('alightPredefinedLinkHref');

    // Setup highlight over selected link.
    inlineHighlight(editor, 'alightPredefinedLinkHref', 'a', HIGHLIGHT_CLASS);
  ...
  private _enableAutomaticDecorators(automaticDecoratorDefinitions: Array<NormalizedLinkDecoratorAutomaticDefinition>): void {
    ...
    // Thanks to that, `AlightPredefinedLinkImageEditing` plugin can re-use the same definitions.
    const command = editor.commands.get('alight-predefined-link') as AlightPredefinedLinkCommand;
   ...
   * {@link module:link/AlightPredefinedLinkCommand~AlightPredefinedLinkCommand#manualDecorators} collection (a model for manual decorators state).
   *
    ...
    manualDecoratorDefinitions.forEach(decoratorDefinition => {
      ...
      editor.conversion.for('downcast').attributeToElement({
        ...
        view: (manualDecoratorValue, { writer, schema }, { item }) => {
          // Manual decorators for block links are handled e.g. in AlightPredefinedLinkImageEditing.
  ...
  /**
   * Watches the DocumentSelection attribute changes and removes link decorator attributes when the alightPredefinedLinkHref attribute is removed.
   *
  ...
  private _enableSelectionAttributesFixer(): void {
    ...
    this.listenTo<DocumentSelectionChangeAttributeEvent>(selection, 'change:attribute', (evt, { attributeKeys }) => {
      if (!attributeKeys.includes('alightPredefinedLinkHref') || selection.hasAttribute('alightPredefinedLinkHref')) {
        return;
      }
    ...
    this.listenTo<ClipboardContentInsertionEvent>(editor.plugins.get('ClipboardPipeline'), 'contentInsertion', (evt, data) => {
      model.change(writer => {
        ...
        for (const item of range.getItems()) {
          if (item.hasAttribute('alightPredefinedLinkHref')) {
            const newLink = addLinkProtocolIfApplicable(item.getAttribute('alightPredefinedLinkHref') as string, defaultProtocol);

            writer.setAttribute('alightPredefinedLinkHref', newLink, item);
          }
        }
      });
    });
 ...
 * All link-related model attributes start with "link". That includes not only "alightPredefinedLinkHref"
 * but also all decorator attributes (they have dynamic names), or even custom plugins.
 */
function removeLinkAttributesFromSelection(writer: Writer, linkAttributes: Array<string>): void {
  writer.removeSelectionAttribute('alightPredefinedLinkHref');
