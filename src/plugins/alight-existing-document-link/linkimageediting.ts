// src/plugins/alight-existing-document-link/linkimageediting.ts
import {
  Plugin,
  type Editor
 ...
 *
 * It accepts the `AlightExistingDocumentLinkPluginHref="url"` attribute in the model for the {@link module:image/image~Image `<imageBlock>`} element
 * which allows linking images.
 */
export default class AlightExistingDocumentLinkPluginImageEditing extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return ['ImageEditing', 'ImageUtils', AlightExistingDocumentLinkPluginEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightExistingDocumentLinkPluginImageEditing' as const;
  }
  ...
  public afterInit(): void {
    ...
    if (editor.plugins.has('ImageBlockEditing')) {
      schema.extend('imageBlock', { allowAttributes: ['AlightExistingDocumentLinkPluginHref'] });
    }

    editor.conversion.for('upcast').add(upcastLink(editor));
    editor.conversion.for('downcast').add(downcastImageLink(editor));

    // Definitions for decorators are provided by the `link` command and the `AlightExistingDocumentLinkPluginEditing` plugin.
...
function upcastLink(editor: Editor): (dispatcher: UpcastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<UpcastElementEvent>('element:a', (evt, data, conversionApi) => {
      ...
      // There's an image inside an <a> element - we consume it so it won't be picked up by the AlightExistingDocumentLinkPlugin plugin.
      ...
      const AlightExistingDocumentLinkPluginHref = viewLink.getAttribute('href');

      // Missing the 'href' attribute.
      if (!AlightExistingDocumentLinkPluginHref) {
        return;
      }
      ...
      if (modelElement && modelElement.is('element', 'imageBlock')) {
        // Set the AlightExistingDocumentLinkPluginHref attribute from link element on model image element.
        conversionApi.writer.setAttribute('AlightExistingDocumentLinkPluginHref', AlightExistingDocumentLinkPluginHref, modelElement);
      }
    }, { priority: 'high' });
...
function downcastImageLink(editor: Editor): (dispatcher: DowncastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>('attribute:AlightExistingDocumentLinkPluginHref:imageBlock', (evt, data, conversionApi) => {
...
function downcastImageLinkManualDecorator(decorator: ManualDecorator): (dispatcher: DowncastDispatcher) => void {
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${decorator.id}:imageBlock`, (evt, data, conversionApi) => {
      ...
      // It may happen when the base `AlightExistingDocumentLinkPluginHref` and decorator attributes are removed
