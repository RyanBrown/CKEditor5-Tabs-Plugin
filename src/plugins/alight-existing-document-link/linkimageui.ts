// src/plugins/alight-existing-document-link/linkimageui.ts
 ...
 *
 * This plugin provides the `'AlightExistingDocumentLinkPluginImage'` button that can be displayed in the {@link module:image/imagetoolbar~ImageToolbar}.
 * It can be used to wrap images in links.
 */
export default class AlightExistingDocumentLinkPluginImageUI extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return [AlightExistingDocumentLinkPluginEditing, AlightExistingDocumentLinkPluginUI, 'ImageBlockEditing'] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightExistingDocumentLinkPluginImageUI' as const;
  }
  ...
  public init(): void {
    ...
    this.listenTo<ViewDocumentClickEvent>(viewDocument, 'click', (evt, data) => {
      if (this._isSelectedLinkedImage(editor.model.document.selection)) {
        ...
        // Block the `AlightExistingDocumentLinkPluginUI` plugin when an image was clicked.
        // In such a case, we'd like to display the image toolbar.
        evt.stop();
      }
    }, { priority: 'high' });

    this._createToolbarAlightExistingDocumentLinkPluginImageButton();
  }

  /**
   * Creates a `AlightExistingDocumentLinkPluginImageUI` button view.
   *
   * Clicking this button shows a modal dialog for editing the image link.
   */
  private _createToolbarAlightExistingDocumentLinkPluginImageButton(): void {
    const editor = this.editor;
    const t = editor.t;

    editor.ui.componentFactory.add('AlightExistingDocumentLinkPluginImage', locale => {
      const button = new ButtonView(locale);
      const plugin = editor.plugins.get('AlightExistingDocumentLinkPluginUI');
      const linkCommand = editor.commands.get('alight-existing-document-link') as AlightExistingDocumentLinkPluginCommand;

      button.set({
        isEnabled: true,
        label: t('AlightExistingDocumentLinkPlugin image'),
        icon: linkIcon,
