...
export default class AutomaticDecorators {
	...
	public getDispatcher(): (dispatcher: DowncastDispatcher) => void {
		return dispatcher => {
			dispatcher.on<DowncastAttributeEvent>('attribute:alightLinkHref', (evt, data, conversionApi) => {
				// There is only test as this behavior decorates links and
				// it is run before dispatcher which actually consumes this node.
				// This allows on writing own dispatcher with highest priority,
				// which blocks both native converter and this additional decoration.
				if (!conversionApi.consumable.test(data.item, 'attribute:alightLinkHref')) {
					return;
				}

				// Automatic decorators for block links are handled e.g. in AlightLinkImageEditing.
				...
				for (const item of this._definitions) {
					...
					viewWriter.setCustomProperty('alight-link', true, viewElement);
	...
	public getDispatcherForLinkedImage(): (dispatcher: DowncastDispatcher) => void {
		return dispatcher => {
			dispatcher.on<DowncastAttributeEvent<Element>>('attribute:alightLinkHref:imageBlock', (evt, data, { writer, mapper }) => {
