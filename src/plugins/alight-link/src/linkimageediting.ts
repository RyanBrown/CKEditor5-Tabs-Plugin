...
/**
 * @module link/AlightLinkImageediting
 */
 ...
 *
 * It accepts the `alightLinkHref="url"` attribute in the model for the {@link module:image/image~Image `<imageBlock>`} element
 * which allows linking images.
 */
export default class AlightLinkImageEditing extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return ['ImageEditing', 'ImageUtils', AlightLinkEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightLinkImageEditing' as const;
  }
  ...
  public afterInit(): void {
    ...
    if (editor.plugins.has('ImageBlockEditing')) {
      schema.extend('imageBlock', { allowAttributes: ['alightLinkHref'] });
    }

    editor.conversion.for('upcast').add(upcastLink(editor));
    editor.conversion.for('downcast').add(downcastImageLink(editor));

    // Definitions for decorators are provided by the `link` command and the `AlightLinkEditing` plugin.
...
function upcastLink(editor: Editor): (dispatcher: UpcastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<UpcastElementEvent>('element:a', (evt, data, conversionApi) => {
      ...
      // There's an image inside an <a> element - we consume it so it won't be picked up by the AlightLink plugin.
      ...
      const alightLinkHref = viewLink.getAttribute('href');
      ...
      if (modelElement && modelElement.is('element', 'imageBlock')) {
        // Set the alightLinkHref attribute from link element on model image element.
        conversionApi.writer.setAttribute('alightLinkHref', alightLinkHref, modelElement);
      }
    }, { priority: 'high' });
...
function downcastImageLink(editor: Editor): (dispatcher: DowncastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>('attribute:alightLinkHref:imageBlock', (evt, data, conversionApi) => {
...
function downcastImageLinkManualDecorator(decorator: ManualDecorator): (dispatcher: DowncastDispatcher) => void {
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${decorator.id}:imageBlock`, (evt, data, conversionApi) => {
      ...
      // It may happen when the base `alightLinkHref` and decorator attributes are removed
