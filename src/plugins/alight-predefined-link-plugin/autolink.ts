// src/plugins/alight-predefined-link-plugin/autolink.ts
 ...
 * The autolink plugin.
 */
export default class AlightPredefinedLinkPluginAutoLink extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return [Delete, AlightPredefinedLinkPluginEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightPredefinedLinkPluginAutoLink' as const;
  }
  ...
  private _expandLinkRange(model: Model, position: Position): Range | null {
    if (position.textNode && position.textNode.hasAttribute('alightPredefinedLinkPluginHref')) {
      return findAttributeRange(position, 'alightPredefinedLinkPluginHref', position.textNode.getAttribute('alightPredefinedLinkPluginHref'), model);
    } else {
      return null;
    }
  }
  ...
  private _enablePasteLinking(): void {
    ...
    const AlightPredefinedLinkPluginCommand = editor.commands.get('link')!;

    clipboardPipeline.on('inputTransformation', (evt, data: ClipboardInputTransformationData) => {
      if (!this.isEnabled || !AlightPredefinedLinkPluginCommand.isEnabled || selection.isCollapsed || data.method !== 'paste') {
      ...
      if (matches && matches[2] === newLink) {
        model.change(writer => {
          ...
          AlightPredefinedLinkPluginCommand.execute(newLink);
        });
  ...
  private _persistAutoLink(url: string, range: Range): void {
    ...
    model.enqueueChange(writer => {
      writer.setAttribute('alightPredefinedLinkPluginHref', url, range);
...
function isLinkAllowedOnRange(range: Range, model: Model): boolean {
  return model.schema.checkAttributeInSelection(model.createSelection(range), 'alightPredefinedLinkPluginHref');
}

function linkIsAlreadySet(range: Range): boolean {
  const item = range.start.nodeAfter;
  return !!item && item.hasAttribute('alightPredefinedLinkPluginHref');
}
