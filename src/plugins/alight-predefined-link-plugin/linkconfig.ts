// src/plugins/alight-predefined-link-plugin/linkconfig.ts
import type { ArrayOrItem } from '@ckeditor/ckeditor5-utils';

/**
 * The configuration of the {@link module:link/link~AlightPredefinedLinkPlugin link feature}.
 *
 * ```ts
 * ClassicEditor
 * 	.create( editorElement, {
 * 		link:  ... // AlightPredefinedLinkPlugin feature configuration.
 * 	} )
 * 	.then( ... )
 * 	.catch( ... );
 * ```
...
export interface LinkConfig {
   ...
   * in the editor. "Predefined links" are all links in the editor content starting with `http`, `https`, or `//`.
   *
   * ```ts
   * ClassicEditor
   * 	.create( editorElement, {
   ...
   *
   * AlightPredefinedLinkPlugin decorators are defined as objects with key-value pairs, where the key is the name provided for a given decorator and the
...
export interface LinkDecoratorAutomaticDefinition {
  ...
  /**
   * AlightPredefinedLinkPlugin decorator type. It is `'automatic'` for all automatic decorators.
   */
  mode: 'automatic';
...
export interface LinkDecoratorManualDefinition {
  ...
  /**
   * AlightPredefinedLinkPlugin decorator type. It is `'manual'` for all manual decorators.
   */
  mode: 'manual';
