// src/plugins/alight-predefined-link-plugin/linkimageediting.ts
import {
  Plugin,
  type Editor
 ...
 *
 * It accepts the `alightPredefinedLinkPluginHref="url"` attribute in the model for the {@link module:image/image~Image `<imageBlock>`} element
 * which allows linking images.
 */
export default class AlightPredefinedLinkPluginImageEditing extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return ['ImageEditing', 'ImageUtils', AlightPredefinedLinkPluginEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightPredefinedLinkPluginImageEditing' as const;
  }
  ...
  public afterInit(): void {
    ...
    if (editor.plugins.has('ImageBlockEditing')) {
      schema.extend('imageBlock', { allowAttributes: ['alightPredefinedLinkPluginHref'] });
    }

    editor.conversion.for('upcast').add(upcastLink(editor));
    editor.conversion.for('downcast').add(downcastImageLink(editor));

    // Definitions for decorators are provided by the `link` command and the `AlightPredefinedLinkPluginEditing` plugin.
...
function upcastLink(editor: Editor): (dispatcher: UpcastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<UpcastElementEvent>('element:a', (evt, data, conversionApi) => {
      ...
      // There's an image inside an <a> element - we consume it so it won't be picked up by the AlightPredefinedLinkPlugin plugin.
      ...
      const alightPredefinedLinkPluginHref = viewLink.getAttribute('href');

      // Missing the 'href' attribute.
      if (!alightPredefinedLinkPluginHref) {
        return;
      }
      ...
      if (modelElement && modelElement.is('element', 'imageBlock')) {
        // Set the alightPredefinedLinkPluginHref attribute from link element on model image element.
        conversionApi.writer.setAttribute('alightPredefinedLinkPluginHref', alightPredefinedLinkPluginHref, modelElement);
      }
    }, { priority: 'high' });
...
function downcastImageLink(editor: Editor): (dispatcher: DowncastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>('attribute:alightPredefinedLinkPluginHref:imageBlock', (evt, data, conversionApi) => {
...
function downcastImageLinkManualDecorator(decorator: ManualDecorator): (dispatcher: DowncastDispatcher) => void {
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${decorator.id}:imageBlock`, (evt, data, conversionApi) => {
      ...
      // It may happen when the base `alightPredefinedLinkPluginHref` and decorator attributes are removed
