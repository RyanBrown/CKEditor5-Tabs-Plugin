...
export default class AlightPredefinedLinkPluginEditing extends Plugin {
  ...
  private _defineSchema(): void {
    ...
    // Register the alightPredefinedLinkPlugin attribute
    schema.extend('$text', {
  ...
  private _defineConverters(): void {
    ...
    conversion.for('downcast').attributeToElement({
      model: {
        key: 'alightPredefinedLinkPlugin', // Model attribute key
    ...
    conversion.for('upcast').elementToAttribute({
      ...
      model: {
        key: 'alightPredefinedLinkPlugin',
  ...
  private _defineCommands(): void {
    const editor = this.editor;
    editor.commands.add('alightPredefinedLinkPlugin', new AlightPredefinedLinkPluginCommand(editor));
  }
}
