...
describe('AlightPredefinedLinkPluginUI', () => {
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightPredefinedLinkPlugin, ContextualBalloon],
  ...
  describe('init()', () => {
    ...
    it('should register toolbar component', () => {
      const button = editor.ui.componentFactory.create('alightPredefinedLinkPlugin');
  ...
  describe('balloon behavior', () => {
    ...
    it('should hide balloon when selection moves outside link', () => {
      setData(
        editor.model,
        `<paragraph><$text alightPredefinedLinkPlugin='${JSON.stringify({
          url: 'https://example.com',
          orgName: 'Example Org'
  ...
  describe('ActionsView', () => {
    ...
    it('should execute unlink command when unlink button is clicked', () => {
      ...
      expect(spy).toHaveBeenCalledWith('alightPredefinedLinkPlugin');
    });
  });
