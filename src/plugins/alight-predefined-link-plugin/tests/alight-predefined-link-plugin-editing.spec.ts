...
describe('AlightPredefinedLinkPluginEditing', () => {
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightPredefinedLinkPluginEditing],
  ...
  it('should set up proper schema rules', () => {
    // Check if the schema allows alightPredefinedLinkPlugin attribute on text
    expect(editor.model.schema.checkAttribute(['$text'], 'alightPredefinedLinkPlugin')).toBe(true);
  });
  ...
  describe('conversion', () => {
    it('should convert model link to view link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightPredefinedLinkPlugin='${JSON.stringify(linkData)}'>foo</$text></paragraph>`
      );
    ...
    //   // Parse the alightPredefinedLinkPlugin attribute value
    //   const match = modelData.match(/alightPredefinedLinkPlugin='([^']+)'/);
