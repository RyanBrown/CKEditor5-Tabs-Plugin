...
import { getSelectedLinkElement, hasLinkAttribute, getLinkAttributeValue } from '../alight-predefined-link-plugin-utils';
...
describe('AlightPredefinedLinkPluginUtils', () => {
  let editor: any;
  ...
  describe('getSelectedLinkElement', () => {
    ...
    it('should return link element when selection is inside link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightPredefinedLinkPlugin='${JSON.stringify(linkData)}'>f[]oo</$text></paragraph>`
      );
      const element = getSelectedLinkElement(editor);
      expect(element).toBeTruthy();
      expect(element?.is('element', 'a')).toBe(true);
