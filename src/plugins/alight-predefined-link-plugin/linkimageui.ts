// src/plugins/alight-predefined-link-plugin/linkimageui.ts
 ...
 *
 * This plugin provides the `'AlightPredefinedLinkPluginImage'` button that can be displayed in the {@link module:image/imagetoolbar~ImageToolbar}.
 * It can be used to wrap images in links.
 */
export default class AlightPredefinedLinkPluginImageUI extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return [AlightPredefinedLinkPluginEditing, AlightPredefinedLinkPluginUI, 'ImageBlockEditing'] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightPredefinedLinkPluginImageUI' as const;
  }
  ...
  public init(): void {
    ...
    this.listenTo<ViewDocumentClickEvent>(viewDocument, 'click', (evt, data) => {
      if (this._isSelectedLinkedImage(editor.model.document.selection)) {
        ...
        // Block the `AlightPredefinedLinkPluginUI` plugin when an image was clicked.
        // In such a case, we'd like to display the image toolbar.
        evt.stop();
      }
    }, { priority: 'high' });

    this._createToolbarAlightPredefinedLinkPluginImageButton();
  }

  /**
   * Creates a `AlightPredefinedLinkPluginImageUI` button view.
   *
   * Clicking this button shows a modal dialog for editing the image link.
   */
  private _createToolbarAlightPredefinedLinkPluginImageButton(): void {
    const editor = this.editor;
    const t = editor.t;

    editor.ui.componentFactory.add('AlightPredefinedLinkPluginImage', locale => {
      const button = new ButtonView(locale);
      const plugin = editor.plugins.get('AlightPredefinedLinkPluginUI');
      const linkCommand = editor.commands.get('alight-predefined-link') as AlightPredefinedLinkPluginCommand;

      button.set({
        isEnabled: true,
        label: t('AlightPredefinedLinkPlugin image'),
        icon: linkIcon,
