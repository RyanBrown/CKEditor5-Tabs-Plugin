...
import { isPredefinedLink, hasAHCustomeLink } from './utils';
  ...
  public init(): void {
    ...
    if (editor.plugins.has('Link')) {
      ...
      standardLinkUI._showUI = function (isEditing: boolean) {
        ...
        if (linkElement && hasAHCustomeLink(linkElement)) {
      ...
      if (standardLinkCommand) {
        standardLinkCommand.on('execute', (evt, args) => {
          ...
          if (selection.hasAttribute('alightPredefinedLinkPluginHref')) {
            ...
            // *** KEY CHANGE: Check both URL format AND AHCustomeLink class ***
            if (isPredefinedLink(currentHref as string) && linkElement && hasAHCustomeLink(linkElement)) {
