...
export default class AlightEmailLinkImageUI extends Plugin {
	 ...
	 *
	 * Clicking this button shows a {@link module:link/linkui~AlightEmailLinkUI#_balloon} attached to the selection.
	 * When an image is already linked, the view shows {@link module:link/linkui~AlightEmailLinkUI#actionsView} or
	 * {@link module:link/linkui~AlightEmailLinkUI#formView} if it is not.
	 */
	private _createToolbarAlightEmailLinkImageButton(): void {
		...
		editor.ui.componentFactory.add('AlightEmailLinkImage', locale => {
			...
			// Show the actionsView or formView (both from AlightEmailLinkUI) on button click depending on whether the image is linked already.
