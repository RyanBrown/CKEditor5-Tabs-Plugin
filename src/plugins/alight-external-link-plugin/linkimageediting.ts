// src/plugins/alight-external-link-plugin/linkimageediting.ts
import {
  Plugin,
  type Editor
...
export default class AlightExternalLinkPluginImageEditing extends Plugin {
  ...
  public afterInit(): void {
    ...
    if (editor.plugins.has('ImageBlockEditing')) {
      schema.extend('imageBlock', { allowAttributes: ['alightExternalLinkHref'] });
    }
...
function upcastLink(editor: Editor): (dispatcher: UpcastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<UpcastElementEvent>('element:a', (evt, data, conversionApi) => {
      ...
      const alightExternalLinkHref = viewLink.getAttribute('href');

      // Missing the 'href' attribute.
      if (!alightExternalLinkHref) {
        return;
      }
      ...
      if (modelElement && modelElement.is('element', 'imageBlock')) {
        // Set the alightExternalLinkHref attribute from link element on model image element.
        conversionApi.writer.setAttribute('alightExternalLinkHref', alightExternalLinkHref, modelElement);
      }
    }, { priority: 'high' });
...
function downcastImageLink(editor: Editor): (dispatcher: DowncastDispatcher) => void {
  ...
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>('attribute:alightExternalLinkHref:imageBlock', (evt, data, conversionApi) => {
...
function downcastImageLinkManualDecorator(decorator: ManualDecorator): (dispatcher: DowncastDispatcher) => void {
  return dispatcher => {
    dispatcher.on<DowncastAttributeEvent<Element>>(`attribute:${decorator.id}:imageBlock`, (evt, data, conversionApi) => {
      ...
      // It may happen when the base `alightExternalLinkHref` and decorator attributes are removed
