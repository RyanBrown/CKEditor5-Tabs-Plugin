// src/plugins/alight-generic-link-plugin/tests/alight-generic-link-plugin-utils.spec.ts
import { getSelectedLinkElement, hasLinkAttribute, getLinkAttributeValue } from '../alight-generic-link-plugin-utils';
...
describe('AlightGenericLinkPluginUtils', () => {
  let editor: any;
  ...
  describe('getSelectedLinkElement', () => {
    ...
    it('should return link element when selection is inside link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightGenericLinkPlugin='${JSON.stringify(linkData)}'>f[]oo</$text></paragraph>`
      );
      const element = getSelectedLinkElement(editor);
      expect(element).toBeTruthy();
      expect(element?.is('element', 'a')).toBe(true);
