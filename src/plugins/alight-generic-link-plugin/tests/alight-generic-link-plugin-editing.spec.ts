// src/plugins/alight-generic-link-plugin/tests/alight-generic-link-plugin-editing.spec.ts
...
describe('AlightGenericLinkPluginEditing', () => {
  let editor: any;
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightGenericLinkPluginEditing],
  ...
  it('should set up proper schema rules', () => {
    // Check if the schema allows alightGenericLinkPlugin attribute on text
    expect(editor.model.schema.checkAttribute(['$text'], 'alightGenericLinkPlugin')).toBe(true);
  });
  ...
  describe('conversion', () => {
    it('should convert model link to view link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightGenericLinkPlugin='${JSON.stringify(linkData)}'>foo</$text></paragraph>`
      );
    ...
    //   // Parse the alightGenericLinkPlugin attribute value
    //   const match = modelData.match(/alightGenericLinkPlugin='([^']+)'/);
