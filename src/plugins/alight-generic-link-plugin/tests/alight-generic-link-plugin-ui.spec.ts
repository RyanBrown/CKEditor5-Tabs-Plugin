// src/plugins/alight-generic-link-plugin/tests/alight-generic-link-plugin-ui.spec.ts
...
describe('AlightGenericLinkPluginUI', () => {
  let editor: any;
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightGenericLinkPlugin, ContextualBalloon],
  ...
  describe('init()', () => {
    ...
    it('should register toolbar component', () => {
      const button = editor.ui.componentFactory.create('alightGenericLinkPlugin');
  ...
  describe('balloon behavior', () => {
    ...
    it('should hide balloon when selection moves outside link', () => {
      setData(
        editor.model,
        `<paragraph><$text alightGenericLinkPlugin='${JSON.stringify({
          url: 'https://example.com',
          orgName: 'Example Org'
  ...
  describe('ActionsView', () => {
    ...
    it('should execute unlink command when unlink button is clicked', () => {
      ...
      expect(spy).toHaveBeenCalledWith('alightGenericLinkPlugin');
    });
  });
  ...
  describe('Modal behavior', () => {
    it('should validate form before applying changes', async () => {
      ...
      const form = document.querySelector('#generic-link-form');
