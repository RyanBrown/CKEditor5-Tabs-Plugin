// src/plugins/alight-generic-link-plugin/alight-generic-link-command.ts
import { Command } from '@ckeditor/ckeditor5-core';
import { Item } from '@ckeditor/ckeditor5-engine';
import { OrganizationNameHandler, normalizeGenericLinkUrl, getSelectedLinkElement } from './alight-generic-link-plugin-utils';

// Interface for command options when applying an email link.
export interface AlightGenericLinkPluginCommandOptions {
  email: string;
  orgName?: string;
}

// Command for applying and managing email links within the editor.
export default class AlightGenericLinkPluginCommand extends Command {
  public readonly orgNameHandler: OrganizationNameHandler;

  /**
   * Creates a new AlightGenericLinkPluginCommand.
   ...
   */
  override execute(options: AlightGenericLinkPluginCommandOptions): void {
    ...
    const normalizedEmail = normalizeGenericLinkUrl(email);
  ...
  override refresh(): void {
    ...
    // AlightGenericLinkPluginCommand should be enabled when the regular link command is enabled
