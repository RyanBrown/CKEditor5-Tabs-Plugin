// src/plugins/alight-generic-link-plugin/alight-generic-link-plugin-command.ts
...
export default class AlightGenericLinkPluginCommand extends Command {
  ...
  override refresh(): void {
    ...
    this.isEnabled = model.schema.checkAttributeInSelection(selection, 'alightGenericLinkPlugin');
    ...
    const attributeValue = selection.getAttribute('alightGenericLinkPlugin');
  ...
  override execute(linkData?: LinkAttributes): void {
    ...
    model.change(writer => {
      ...
      if (!linkData) {
        const ranges = selection.isCollapsed
          ? [findAttributeRange(
            ...
            selection.getAttribute('alightGenericLinkPlugin'),
            model
          )]
          : selection.getRanges();
        ...
        for (const range of ranges) {
          ...
          writer.removeAttribute('alightGenericLinkPlugin', range);
        }
        return;
      }
      ...
      if (selection.isCollapsed) {
        ...
        const ranges = model.schema.getValidRanges(selection.getRanges(), 'alightGenericLinkPlugin');
