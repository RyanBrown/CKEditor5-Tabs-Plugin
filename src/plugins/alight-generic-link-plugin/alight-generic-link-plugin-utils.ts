// src/plugins/alight-generic-link-plugin/alight-generic-link-plugin-utils.ts
...
/**
 * Checks if the selection has the 'alightGenericLinkPlugin' attribute.
 * This attribute is used to indicate that a public link is applied.
 *
 * @param selection - The current selection in the editor.
 * @returns True if the selection has the 'alightGenericLinkPlugin' attribute, false otherwise.
 */
export function hasLinkAttribute(selection: any): boolean {
  return selection.hasAttribute('alightGenericLinkPlugin');
}

/**
 * Retrieves the value of the 'alightGenericLinkPlugin' attribute from the selection.
...
export function getLinkAttributeValue(selection: any): { url: string; orgName?: string } | undefined {
  const value = selection.getAttribute('alightGenericLinkPlugin');
