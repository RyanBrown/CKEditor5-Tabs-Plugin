// src/plugins/alight-predefined-link-plugin/tests/alight-predefined-link-plugin-editing.spec.ts
...
describe('AlightExistingDocumentLinkPluginEditing', () => {
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightExistingDocumentLinkPluginEditing],
  ...
  it('should set up proper schema rules', () => {
    // Check if the schema allows alightExistingDocumentLinkPlugin attribute on text
    expect(editor.model.schema.checkAttribute(['$text'], 'alightExistingDocumentLinkPlugin')).toBe(true);
  });
  ...
  describe('conversion', () => {
    it('should convert model link to view link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightExistingDocumentLinkPlugin='${JSON.stringify(linkData)}'>foo</$text></paragraph>`
      );
    ...
    //   // Parse the alightExistingDocumentLinkPlugin attribute value
    //   const match = modelData.match(/alightExistingDocumentLinkPlugin='([^']+)'/);
