// src/plugins/alight-existing-document-link/autolink.ts
 ...
 * The autolink plugin.
 */
export default class AlightExistingDocumentLinkPluginAutoLink extends Plugin {
  /**
   * @inheritDoc
   */
  public static get requires() {
    return [Delete, AlightExistingDocumentLinkPluginEditing] as const;
  }
  ...
  public static get pluginName() {
    return 'AlightExistingDocumentLinkPluginAutoLink' as const;
  }
  ...
  private _enablePasteLinking(): void {
    ...
    const AlightExistingDocumentLinkPluginCommand = editor.commands.get('link')!;

    clipboardPipeline.on('inputTransformation', (evt, data: ClipboardInputTransformationData) => {
      if (!this.isEnabled || !AlightExistingDocumentLinkPluginCommand.isEnabled || selection.isCollapsed || data.method !== 'paste') {
      ...
      if (matches && matches[2] === newLink) {
        model.change(writer => {
          ...
          AlightExistingDocumentLinkPluginCommand.execute(newLink);
        });
