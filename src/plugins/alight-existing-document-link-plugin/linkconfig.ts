// src/plugins/alight-existing-document-link/linkconfig.ts
import type { ArrayOrItem } from '@ckeditor/ckeditor5-utils';

/**
 * The configuration of the {@link module:link/link~AlightExistingDocumentLinkPlugin link feature}.
 *
 * ```ts
 * ClassicEditor
 * 	.create( editorElement, {
 * 		link:  ... // AlightExistingDocumentLinkPlugin feature configuration.
 * 	} )
 * 	.then( ... )
 * 	.catch( ... );
 * ```
...
export interface LinkConfig {
   ...
   * in the editor. "Existing Document links" are all links in the editor content starting with `http`, `https`, or `//`.
   *
   * ```ts
   * ClassicEditor
   * 	.create( editorElement, {
   ...
   * ```
   *
   * Internally, this option activates a existing document {@link module:link/linkconfig~LinkConfig#decorators automatic link decorator}
   ...
   *
   * AlightExistingDocumentLinkPlugin decorators are defined as objects with key-value pairs, where the key is the name provided for a given decorator and the
   ...
   *
   * **Note**: Since the `target` attribute management for external links is a common use case, there is a existing document automatic decorator
...
export interface LinkDecoratorAutomaticDefinition {
  ...
  /**
   * AlightExistingDocumentLinkPlugin decorator type. It is `'automatic'` for all automatic decorators.
   */
  mode: 'automatic';
 ...
 * of a existing document set of attributes.
 *
...
export interface LinkDecoratorManualDefinition {
  ...
  /**
   * AlightExistingDocumentLinkPlugin decorator type. It is `'manual'` for all manual decorators.
   */
  mode: 'manual';
