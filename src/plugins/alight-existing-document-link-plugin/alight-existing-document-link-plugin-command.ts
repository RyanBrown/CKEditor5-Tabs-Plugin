...
export default class AlightExistingDocumentLinkPluginCommand extends Command {
  ...
  override refresh(): void {
    ...
    this.isEnabled = model.schema.checkAttributeInSelection(selection, 'alightExistingDocumentLinkPlugin');
    ...
    const attributeValue = selection.getAttribute('alightExistingDocumentLinkPlugin');
  ...
  override execute(linkData?: LinkAttributes): void {
    ...
    model.change(writer => {
      ...
      if (!linkData) {
        const ranges = selection.isCollapsed
          ? [findAttributeRange(
            ...
            selection.getAttribute('alightExistingDocumentLinkPlugin'),
            model
          )]
          : selection.getRanges();
        ...
        for (const range of ranges) {
          ...
          writer.removeAttribute('alightExistingDocumentLinkPlugin', range);
        }
        return;
      }
      ...
      if (selection.isCollapsed) {
        ...
        const ranges = model.schema.getValidRanges(selection.getRanges(), 'alightExistingDocumentLinkPlugin');
