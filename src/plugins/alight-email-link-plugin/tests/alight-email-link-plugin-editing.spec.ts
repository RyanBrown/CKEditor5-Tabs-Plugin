...
describe('AlightEmailLinkPluginEditing', () => {
  ...
  beforeEach(async () => {
    ...
    editor = await ClassicEditor.create(element, {
      plugins: [Essentials, Paragraph, Link, AlightEmailLinkPluginEditing],
  ...
  it('should set up proper schema rules', () => {
    // Check if the schema allows alightEmailLinkPlugin attribute on text
    expect(editor.model.schema.checkAttribute(['$text'], 'alightEmailLinkPlugin')).toBe(true);
  });
  ...
  describe('conversion', () => {
    it('should convert model link to view link', () => {
      ...
      setData(
        editor.model,
        `<paragraph><$text alightEmailLinkPlugin='${JSON.stringify(linkData)}'>foo</$text></paragraph>`
      );
    ...
    //   // Parse the alightEmailLinkPlugin attribute value
    //   const match = modelData.match(/alightEmailLinkPlugin='([^']+)'/);
