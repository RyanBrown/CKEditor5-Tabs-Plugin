...
export default class AlightNewDocumentLinkPluginCommand extends Command {
  ...
  override refresh(): void {
    ...
    this.isEnabled = model.schema.checkAttributeInSelection(selection, 'AlightNewDocumentLinkPlugin');
    ...
    const attributeValue = selection.getAttribute('alightNewDocumentLinkPlugin');
  ...
  override execute(linkData?: LinkAttributes): void {
    ...
    model.change(writer => {
      ...
      if (!linkData) {
        const ranges = selection.isCollapsed
          ? [findAttributeRange(
            ...
            selection.getAttribute('alightNewDocumentLinkPlugin'),
            model
          )]
          : selection.getRanges();
        ...
        for (const range of ranges) {
          ...
          writer.removeAttribute('alightNewDocumentLinkPlugin', range);
        }
        return;
      }
      ...
      if (selection.isCollapsed) {
        ...
        writer.setAttribute('alightNewDocumentLinkPlugin', linkData, range);
      } else {
        // If the selection is not collapsed, apply attributes to the selected range
        const ranges = model.schema.getValidRanges(selection.getRanges(), 'alightNewDocumentLinkPlugin');
