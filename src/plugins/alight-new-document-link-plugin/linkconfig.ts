// src/plugins/alight-new-document-link-plugin/linkconfig.ts
import type { ArrayOrItem } from '@ckeditor/ckeditor5-utils';

/**
 * The configuration of the {@link module:link/link~AlightNewDocumentLinkPlugin link feature}.
 *
 * ```ts
 * ClassicEditor
 * 	.create( editorElement, {
 * 		link:  ... // AlightNewDocumentLinkPlugin feature configuration.
 * 	} )
 * 	.then( ... )
 * 	.catch( ... );
 * ```
 ...
 */
export interface AlightNewDocumentLinkConfig {
   ...
   *
   * AlightNewDocumentLinkPlugin decorators are defined as objects with key-value pairs, where the key is the name provided for a given decorator and the
   * value is the decorator definition.
   *
   * The name of the decorator also corresponds to the {@glink framework/architecture/editing-engine#text-attributes text
   * attribute} in the model. For instance, the `isEmail` decorator below is represented as a `linkIsNewDocument` attribute in the model.
   *
   * ```ts
   * ClassicEditor
   * 	.create( editorElement, {
...
export interface LinkDecoratorAutomaticDefinition {
  ...
  /**
   * AlightNewDocumentLinkPlugin decorator type. It is `'automatic'` for all automatic decorators.
   */
  mode: 'automatic';
...
export interface LinkDecoratorManualDefinition {
  ...
  /**
   * AlightNewDocumentLinkPlugin decorator type. It is `'manual'` for all manual decorators.
   */
  mode: 'manual';
