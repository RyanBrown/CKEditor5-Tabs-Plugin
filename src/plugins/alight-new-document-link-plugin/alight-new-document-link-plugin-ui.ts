...
import { createPublicLinkModalContent, validateForm } from './modal-content/new-document';
import type AlightNewDocumentLinkPluginCommand from './alight-new-document-link-plugin-command';
...
export default class AlightNewDocumentLinkPluginUI extends Plugin {
  ...
  private _setupToolbarButton(): void {
    ...
    editor.ui.componentFactory.add('alightNewDocumentLinkPlugin', locale => {
      ...
      if (!command) {
        console.warn('AlightNewDocumentLinkPlugin command not found');
        return button;
      }
  ...
  private _createActionsView(): ActionsView {
    ...
    const command = editor.commands.get('alightNewDocumentLinkPlugin') as AlightNewDocumentLinkPluginCommand;
  ...
  private _showBalloon(): void {
    ...
    const command = this.editor.commands.get('alightNewDocumentLinkPlugin') as AlightNewDocumentLinkPluginCommand;
  ...
  // In AlightNewDocumentLinkPluginUI class, change the _showModal method to:
  ...
  public _showModal(initialValue?: { url: string; orgName?: string }): void {
    ...
    if (!this._modalDialog) {
      ...
      this._modalDialog.on('buttonClick', (label: string) => {
        ...
        if (label === 'Continue') {
          const form = this._modalDialog?.element?.querySelector('#new-document-form') as HTMLFormElement;
