...
//       (mockedExternalPlugin.showUI as jasmine.Spy).and.throwError('No showUI');
...
//       expect(warnSpy).toHaveBeenCalledWith(jasmine.stringMatching(/No UI method or command found for plugin AlightEmailLinkPlugin/));
//     });
...
//       const modalSpy = spyOn(modalOnlyPlugin, '_showModal').and.callThrough();
...
//       const modalSpy = spyOn(errorPlugin, '_showModal'); // track if fallback _showModal would be called
//       spyOn(editor.plugins, 'get').and.callFake((name: string): any => {
//         if (name === 'AlightExternalLinkPluginUI') {
//           return null;
//         }
...
//       expect(errorSpy.calls.argsFor(0)[0]).toContain(`Error showing UI for ${externalConfig.name}:`);
